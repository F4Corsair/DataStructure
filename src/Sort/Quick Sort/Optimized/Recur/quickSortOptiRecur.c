...
{
    // compare 3 elements, smallest one placed on list[left], medium value will be pivot
    ...
    // range : left + 1 ~ right - 2 (3 elements above are already sorted)
    int lptr = left + 1, rptr = right - 2;
    do
    {
...
{
    // compare 3 elements, biggest one placed on list[left], medium value will be pivot
    if (list[right - 1] < list[right])
        swap(list, right - 1, right);
    if (list[left] < list[right - 1])
        swap(list, left, right - 1);
